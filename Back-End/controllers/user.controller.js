...
export const register = async (req, res) => {
    try {
        const { 
            ...
            title, bio, skills, education, experience, projects, hourlyRate, availability
        } = req.body;
        ...
        // Add researcher profile if role is freelancer
        if (role === 'freelancer') {
...
// Get Researcher Profile (Freelancer only)
export const getResearcherProfile = async (req, res) => {
  try {
    const userId = req.id;

    const user = await User.findById(userId).select('researcherProfile role fullname email phoneNumber');
    ...
    if (user.role !== 'freelancer') {
      return res.status(403).json({
        message: "Only freelancers can access researcher profile",
        success: false
      });
    }
    ...
    return res.status(200).json({
      ...
      profileCompleted: user.researcherProfile?.profileCompleted || false,
      userInfo: {
        fullname: user.fullname,
        email: user.email,
        phoneNumber: user.phoneNumber
...
// Update Researcher Profile (Freelancer only)
export const updateResearcherProfile = async (req, res) => {
  try {
    ...
    if (user.role !== 'freelancer') {
      return res.status(403).json({
        message: "Only freelancers can update researcher profile",
        success: false
      });
    }

    // Update researcher profile with completion status
